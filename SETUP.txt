...
Access to GitHub site features is problematic with the Raspberry Pi browser. I
needed to generate my SSH key on the Raspberry Pi and then email to myself. I
could then input the key in the GitHub settings via a browser on my Mac.

Note: At this point during my thrid MarkBot install, the double-quote and
at-sign characters on my keyboard were swapped.
...
# perform separately, seems to cause problems during full npm install
# sudo npm --save install raspi-io
# INSTEAD, INSTALL GLOBALLY AND AVOID HAVING TO INSTALL ON NON-RPI PLATFORMS
...
# Ugh, still trying to fix this. Let's just install raspi-io separately and
# locally on RPi. From markbots home directory:
